...
/// <summary>
    ...
    internal class EventedStreamReader
    {
        ...
        private async Task Run()
        {
            ...
            while (true)
            {
                int chunkLength = await _streamReader.ReadAsync(buf, 0, buf.Length);
                if (chunkLength == 0)
                {
                    OnClosed();
                    break;
                ...
                int lineBreakPos = Array.IndexOf(buf, '\n', 0, chunkLength);
                if (lineBreakPos < 0)
                {
        ...
        {
            OnReceivedChunkHandler dlg = OnReceivedChunk;
            dlg?.Invoke(chunk);
        }

        private void OnCompleteLine(string line)
        {
            OnReceivedLineHandler dlg = OnReceivedLine;
            dlg?.Invoke(line);
        }
